''' Iaxclient wrapper Library by Brandon Kruse '''

from ctypes import *

''' Iaxclient Def's '''
IAXC_FILTER_CN = (1<<4)
IAXC_FORMAT_G723_1 = (1 = << = 0)
IAXC_CALL_STATE_TRANSFER = (1<<7)
IAXC_TEXT_TYPE_ERROR = 3
IAXC_AD_OUTPUT = (1<<1)
IAXC_TEXT_TYPE_NOTICE = 2
IAXC_FILTER_AGC = (1<<1)
IAXC_URL_LDCOMPLETE = 2
IAXC_FORMAT_MPEG4 = (1 = << = 22)
IAXC_REGISTRATION_REPLY_TIMEOUT = 6
IAXC_EVENT_REGISTRATION = 8
IAXC_FORMAT_PNG = (1 = << = 17)
IAXC_AD_OUTPUT_DEFAULT = (1<<4)
IAXC_AD_INPUT = (1<<0)
IAXC_AD_RING_DEFAULT = (1<<5)
IAXC_FORMAT_MAX_VIDEO = (1 = << = 24)
IAXC_FORMAT_SPEEX = (1 = << = 9)
IAXC_EVENT_BUFSIZ = 256
IAXC_CALL_STATE_RINGING = (1<<3)
IAXC_FORMAT_GSM = (1 = << = 1)
IAXC_FORMAT_ADPCM = (1 = << = 5)
IAXC_EVENT_LEVELS = 2
IAXC_URL_LINKURL = 3
IAXC_FORMAT_JPEG = (1 = << = 16)
IAXC_FORMAT_G726 = (1 = << = 4)
IAXC_FORMAT_H261 = (1 = << = 18)
IAXC_FORMAT_H263 = (1 = << = 19)
IAXC_FORMAT_H264 = (1 = << = 21)
IAXC_SOURCE_REMOTE = 2
IAXC_FILTER_ECHO = (1<<2)
IAXC_VIDEO_FORMAT_MASK = (((1<<25)-1) = & = ~IAXC_AUDIO_FORMAT_MASK)
IAXC_TEXT_TYPE_IAX = 5
IAXC_FORMAT_ILBC = (1 = << = 10)
IAXC_FILTER_AAGC = (1<<3)
IAXC_CALL_STATE_SELECTED = (1<<5)
IAXC_CALL_STATE_OUTGOING = (1<<2)
IAXC_CALL_STATE_ACTIVE = (1<<1)
IAXC_AD_RING = (1<<2)
IAXC_SOURCE_LOCAL = 1
IAXC_TEXT_TYPE_FATALERROR = 4
IAXC_URL_URL = 1
IAXC_EVENT_URL = 5
IAXC_EVENT_NETSTAT = 4
IAXC_REGISTRATION_REPLY_ACK = 18
IAXC_EVENT_VIDCAP_ERROR = 12
IAXC_EVENT_DTMF = 9
IAXC_FORMAT_G729A = (1 = << = 8)
IAXC_URL_LINKREJECT = 4
IAXC_FORMAT_H263_PLUS = (1 = << = 20)
IAXC_CALL_STATE_BUSY = (1<<6)
IAXC_FORMAT_ALAW = (1 = << = 3)
IAXC_URL_UNLINK = 5
IAXC_EVENT_AUDIO = 10
IAXC_AUDIO_FORMAT_MASK = ((1<<16)-1)
IAXC_CALL_STATE_FREE = 0
IAXC_EVENT_STATE = 3
IAXC_EVENT_VIDEOSTATS = 11
IAXC_REGISTRATION_REPLY_REJ = 30
IAXC_EVENT_TEXT = 1
IAXC_FORMAT_THEORA = (1 = << = 24)
IAXC_FORMAT_MAX_AUDIO = (1 = << = 15)
IAXC_EVENT_VIDCAP_DEVICE = 13
IAXC_TEXT_TYPE_STATUS = 1
IAXC_FORMAT_ULAW = (1 = << = 2)
IAXC_CALL_STATE_COMPLETE = (1<<4)
IAXC_FORMAT_LPC10 = (1 = << = 7)
IAXC_FILTER_DENOISE = (1<<0)
IAXC_EVENT_VIDEO = 6
IAXC_AD_INPUT_DEFAULT = (1<<3)
IAXC_FORMAT_SLINEAR = (1 = << = 6)
''' End Defs '''

''' iaxc_event '''
class iaxc_event(Structure):
        _fields_ = [("type", c_int)]

IAXC_CB_TYPE = CFUNCTYPE(iaxc_event)
''' End iaxc_event '''
