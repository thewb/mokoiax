...
/* You HAVE to use ALSA on the moko...good idea :] */
//#ifdef AUDIO_ALSA
...
EXPORT int iaxc_initialize(int num_calls)
{
	...
	if(debug) { printf(" calling os_init in iaxc_initialize\n"); }
	os_init();

	if(debug) { printf(" calling jb_output in iaxc_initialize\n"); }
	setup_jb_output();

	if(debug) { printf(" calling MUTEEXINIT in iaxc_initialize\n"); }
	MUTEXINIT(&iaxc_lock);
	MUTEXINIT(&event_queue_lock);

	if(debug) { printf(" calling set audio prefs in iaxc_initialize\n"); }
	iaxc_set_audio_prefs(0);

	if(debug) { printf(" calling set networking prefs in iaxc_initialize\n"); }
	 ...
	 */
	if(debug) { printf(" calling set source_udp_port in iaxc_initialize\n"); }
	...
	if(debug) { printf("Using hacked alsa call....hopefully this will bypass the libportaudio...\n"); }
#ifndef AUDIO_ALSA
	if(debug) { printf(" calling non alsa initialize in iaxc_initialize\n"); }
	 ...
	 */
	if(debug) { printf(" calling alsa audio initialize :D in iaxc_initialize\n"); }
